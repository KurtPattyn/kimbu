 ...
 * @param {!MessageBusTransport} provider - Messagebus transport provider.
 * @class
*/
...
MessageRecipient.prototype.consume = function(options, callback, successCallback) {
  this._provider.consume(this._name, options, callback, successCallback);
};
 ...
 * backend fabric. The physical transmission medium is defined by the supplied MessageBusTransport.
 *
 * @class
 *
 * @param {!MessageBusTransport} provider - Represents the physical transmission medium.
 *
 * @abstract
 */
 ...
 * {@link MessageBusTransport#dispatchChannel}.</p>
 *
 * @class
 * @constructor
 ...
 * @param {!MessageBusTransport} provider - The message bus transport provider.
 * @public
 * @extends {Channel}
...
function DispatchChannel(distributorName, replyRecipientName, provider) {
  ...
  //Not using instanceof because we need to require the MessageBusTransport file
  //and this leads to circular include references
  assert.strictEqual(provider.constructor.super_.name, "MessageBusTransport");
