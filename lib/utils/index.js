 ...
 *
 * @example
 * var target = utils.extend({ a: 1, b: 2 }, { a: 3 }, { c: 5 });
...
function extend(objects) {
  ...
  Object.getOwnPropertyNames(firstObject).forEach(function(propName) {
...
function findModules(folder, type, done) {
  ...
  fs.readdir(folder, function(err, files) {
    ...
    forEach(files, function(file) {
      var current = path.resolve(folder, file);
      var module = require(current);

      if (module.super_ && module.super_ === type) {
