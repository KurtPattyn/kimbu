...
function _rabbitMQMessageToMessage(rmqMessage) {
  ...
  if (util.isBuffer(rmqMessage.content)) {
    try {
      ...
      console.warn("Error parsing '" + rmqMessage.content + "':", err);
      /* istanbul ignore next */
      return null;
    }
  } else {
    //should not happen; let's handle this in a safe way by ignoring the content
    console.warn("RabbitMQ message ignored because it is not a Buffer:", rmqMessage.content);
    parameters = {};
  }
...
RabbitMQTransport.prototype._doConnect = function(callback) {
  //MessageBusTransport.connect() guarantees that it doesn't call _doConnect if there is already
  //a valid connection
  ...
  console.info("Connecting to RabbitMQ host @ %s:%d (URI: %s).", amqpHost, amqpPort, amqpURI);
...
// jscs:enable jsDoc
    if (err) {
      ...
      console.info("Connected to RabbitMQ host @ %s:%d.", amqpHost, amqpPort);
      ...
      console.info("Creating AMQP channel.");
...
// jscs:enable jsDoc
        ...
        if (err) {
          ...
          console.info("Created AMQP channel.");
...
RabbitMQTransport.prototype._doDisconnect = function(callback) {
  //MessageBusTransport.disconnect() guarantees that it doesn't call _doDisconnect if the
...
RabbitMQTransport.prototype._doGetDispatchChannel = function(dispatchChannelName, opts, callback) {
  ...
  console.info("Creating dispatch channel %s on RabbitMQ server.", dispatchChannelName);
...
RabbitMQTransport.prototype._doPublish = function(exchangeName, msg, routingKey, callback) {
  ...
  console.info("Publishing message %j to %s.", msg, exchangeName.length === 0 ? routingKey :
                                                                                exchangeName);
  try {
...
RabbitMQTransport.prototype._doPurge = function(queueName, callback) {
  try {
    this._channel.purgeQueue(queueName, function(err, reply) {
      console.info("Purged %d messages from queue %s", reply.messageCount, queueName);
      callback(err);
    });
  } catch(err) {
    callback(err);
...
RabbitMQTransport.prototype._doConsume = function(queueName, options, callback, successCallback) {
  ...
  console.info("Consuming from queue %s.", queueName);
